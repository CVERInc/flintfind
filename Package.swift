...
// flintfind's engine, as a package rather than as app-internal source: the library
// is the single implementation, and every door — the CLI here, marktile's editor later — reads THAT
// rather than a copy of it. A sibling project's Package.swift carries the scar this avoids; its app and its
...
// `swift test` cannot run here at all. A sibling project reached the same place and ships its gate as an
...
//
// ONE DELIBERATE DEVIATION. the sibling projects name each executable after its package;
